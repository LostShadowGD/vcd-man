...
temp/temp.mpg\necho Finished converting video', shell=True)
        ...
        p = Popen(f'bchunk -r temp/temp.bin temp/temp.cue "{currentDatetime}/{sourceName}"', shell=True)
        p.communicate()
        if isSavingTempFiles:
            status = "Finished making ISOs."
            sleepTime(1)
...
class MenuBar:
    ...
    def start():
        ...
        YESSavingTempFilesButton = PyQt6.QtGui.QAction("True", menuBar)
        YESSavingTempFilesButton.triggered.connect(lambda: MenuBar.toggleSavingTempFiles(True))
        ToggleSavingTempFilesMenu.addAction(YESSavingTempFilesButton)

        NOSavingTempFilesButton = PyQt6.QtGui.QAction("False", menuBar)
        NOSavingTempFilesButton.triggered.connect(lambda: MenuBar.toggleSavingTempFiles(False))
        ToggleSavingTempFilesMenu.addAction(NOSavingTempFilesButton)
